...
Part 5 synchronized React with outside systems. 今篇集中講 **children, composition, context, custom hooks, provider patterns**。目標唔係背 API 名，而係建立一個腦內地圖：當你睇到其他人寫嘅 React code，你可以估到下一步 React 會做咩。
...
| Term | 新手理解 |
| --- | --- |
| `children` | A component can accept JSX inside its opening and closing tags. 呢個 term 我哋先保留英文，因為你之後睇官方 docs、Stack Overflow、GitHub issue 都會見到同一個字。 |
| `composition` | Prefer passing components and elements instead of creating rigid inheritance structures. 呢個 term 我哋先保留英文，因為你之後睇官方 docs、Stack Overflow、GitHub issue 都會見到同一個字。 |
| `Context` | Share values across a subtree without prop drilling. 呢個 term 我哋先保留英文，因為你之後睇官方 docs、Stack Overflow、GitHub issue 都會見到同一個字。 |
| `custom Hook` | Extract reusable stateful logic into a function whose name starts with use. 呢個 term 我哋先保留英文，因為你之後睇官方 docs、Stack Overflow、GitHub issue 都會見到同一個字。 |
