---
title: "ReactJS From Zero Part 9: SSR, Server Components, cache, cacheSignal, and Partial Pre-rendering | ReactJS 從零開始之九：SSR、Server Components、cache、cacheSignal 同 Partial Pre-rendering"
...
> Series navigation: **Part 9 of 10**  
> Previous: Part 8 covered async UI on the client side.  
...
Client rendering is cooking at the customer's table. SSR is preparing the first plate in the kitchen so the customer sees food immediately. Server Components go further: some ingredients never leave the kitchen at all.
...
| --- | --- |
| `SSR` | Render HTML on the server, then hydrate it on the client. |
| `Server Components` | Components that run on the server and can access server-only resources through a framework. |
...
### Debugging Checklist for Part 9
