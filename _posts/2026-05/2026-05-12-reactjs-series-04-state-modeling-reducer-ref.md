...
Part 3 built real forms and lists. 今篇集中講 **state shape, reducer, refs, immutable updates, derived state**。目標唔係背 API 名，而係建立一個腦內地圖：當你睇到其他人寫嘅 React code，你可以估到下一步 React 會做咩。
...
| Term | 新手理解 |
| --- | --- |
| `state shape` | Keep one source of truth and avoid storing values you can calculate. 呢個 term 我哋先保留英文，因為你之後睇官方 docs、Stack Overflow、GitHub issue 都會見到同一個字。 |
| `useReducer` | Useful when updates have names, branches, or multiple fields changing together. 呢個 term 我哋先保留英文，因為你之後睇官方 docs、Stack Overflow、GitHub issue 都會見到同一個字。 |
| `useRef` | Stores a mutable value that survives renders without causing another render. 呢個 term 我哋先保留英文，因為你之後睇官方 docs、Stack Overflow、GitHub issue 都會見到同一個字。 |
| `immutability` | Create new objects and arrays instead of changing existing state in place. 呢個 term 我哋先保留英文，因為你之後睇官方 docs、Stack Overflow、GitHub issue 都會見到同一個字。 |
