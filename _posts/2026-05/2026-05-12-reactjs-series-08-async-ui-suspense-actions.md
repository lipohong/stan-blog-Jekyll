...
Part 7 made render performance measurable and understandable. 今篇集中講 **Suspense, lazy, useTransition, useDeferredValue, useActionState, useOptimistic**。目標唔係背 API 名，而係建立一個腦內地圖：當你睇到其他人寫嘅 React code，你可以估到下一步 React 會做咩。
...
| Term | 新手理解 |
| --- | --- |
| `Suspense` | Show a fallback while a child waits for async work that integrates with Suspense. 呢個 term 我哋先保留英文，因為你之後睇官方 docs、Stack Overflow、GitHub issue 都會見到同一個字。 |
| `lazy` | Load component code only when needed. 呢個 term 我哋先保留英文，因為你之後睇官方 docs、Stack Overflow、GitHub issue 都會見到同一個字。 |
| `transition` | Mark non-urgent updates so urgent typing and clicking stay responsive. 呢個 term 我哋先保留英文，因為你之後睇官方 docs、Stack Overflow、GitHub issue 都會見到同一個字。 |
| `useOptimistic/useActionState` | React 19 APIs for form-like async flows and optimistic UI. 呢個 term 我哋先保留英文，因為你之後睇官方 docs、Stack Overflow、GitHub issue 都會見到同一個字。 |
