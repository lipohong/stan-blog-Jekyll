---
title: "ReactJS From Zero Part 8: Async UI, Suspense, Transitions, Actions, and Optimistic Updates | ReactJS 從零開始之八：Async UI、Suspense、Transitions、Actions 同 Optimistic Updates"
...
> Series navigation: **Part 8 of 10**  
> Previous: Part 7 made render performance measurable and understandable.  
> Next: Part 9 moves from client-only apps into SSR, Server Components, cache, cacheSignal, and partial pre-rendering.  
...
Part 7 made render performance measurable and understandable. In this part we focus on **Suspense, lazy, useTransition, useDeferredValue, useActionState, useOptimistic**. The goal is not to memorize API names. The goal is to build a mental model that helps you read code written by another developer and predict what React will do next.

Async UI is like ordering food while the kitchen is busy. A good restaurant does not freeze the cashier; it gives you a receipt, lets other customers move, and updates the board when the dish is ready.
...
| --- | --- |
| `Suspense` | Show a fallback while a child waits for async work that integrates with Suspense. |
| `lazy` | Load component code only when needed. |
| `transition` | Mark non-urgent updates so urgent typing and clicking stay responsive. |
| `useOptimistic/useActionState` | React 19 APIs for form-like async flows and optimistic UI. |
...
For this post, keep these names in your head: Suspense, lazy, transition, useOptimistic/useActionState. Every code example below is just another way of combining those same ideas.
...
function SearchPage({ records }: { records: RecordItem[] }) {
  ...
  const results = useMemo(() => {
    return records.filter(record =>
      record.title.toLowerCase().includes(deferredQuery.toLowerCase())
    );
...
function TabSwitcher() {
  ...
  return (
    <>
      ...
      {isPending && <p>Preparing screen...</p>}
...
async function addCommentAction(
  ...
  if (text.length < 2) return { error: 'Comment is too short.' };
  await fetch('/api/comments', { method: 'POST', body: formData });
  return { error: null };
}

function CommentForm({ comments }: { comments: Comment[] }) {
  const [state, formAction, isPending] = useActionState(addCommentAction, { error: null });
  ...
  function submit(formData: FormData) {
    addOptimisticComment(String(formData.get('text') ?? ''));
    formAction(formData);
  }
...
4. Add one tiny feature that prepares you for the next post: Part 9 moves from client-only apps into SSR, Server Components, cache, cacheSignal, and partial pre-rendering.
...
### Debugging Checklist for Part 8
...
This part is one step in the series. Keep the code small, verify it in the browser, and continue with the next topic: Part 9 moves from client-only apps into SSR, Server Components, cache, cacheSignal, and partial pre-rendering. The important habit is steady practice: read the component boundary, identify props and state, trigger one event, and confirm the next render.
...
上一篇我哋將 render performance 變成可以量度同理解。今篇集中講 **Suspense, lazy, useTransition, useDeferredValue, useActionState, useOptimistic**。目標唔係背 API 名，而係建立一個腦內地圖：當你睇到其他人寫嘅 React code，你可以估到下一步 React 會做咩。

Async UI 好似廚房忙緊時落單。好嘅餐廳唔會令收銀機死機；佢會俾張飛你、繼續處理其他客，餐到咗再更新叫號牌。
...
| Term | 新手理解 |
| --- | --- |
| `Suspense` | `Suspense` 可以喺 child 等 async work 時顯示 fallback UI。 |
| `lazy` | `lazy` 可以等到需要某個 component 時先載入佢嘅 code。 |
| `transition` | `transition` 標記非緊急 update，令 typing/clicking 呢啲 urgent interaction 保持順。 |
| `useOptimistic/useActionState` | React 19 用嚟處理 form-like async flow 同 optimistic UI 嘅 APIs。 |
...
4. 加入一個好細嘅 feature，預備下一篇會講嘅內容：第九篇會由 client-only apps 走到 SSR、Server Components、cache、cacheSignal 同 partial pre-rendering。
...
今篇係系列入面其中一步。保持 code 細、喺 browser 入面驗證，然後繼續下一個 topic：第九篇會由 client-only apps 走到 SSR、Server Components、cache、cacheSignal 同 partial pre-rendering。 最重要嘅習慣係穩定練習：讀 component boundary、分清 props 同 state、觸發一個 event，再確認下一次 render 係咪符合預期。
